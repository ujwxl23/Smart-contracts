...
contract DeviceShare {
    ...
    function getDeviceByDeviceID(
        uint256 _deviceID
    ) public view returns (string memory, uint256, uint256,string memory,uint256, uint256, bool) {
    ...
    function RequestDeviceUse(uint256 _deviceID, uint256 _hrsToStake) external isListed(_deviceID) {
        uint256 stakedAmountByRequestor = poolContract
            ...
            require(thisDevice.recipient!=msg.sender,"Could not request your own device");
        require(stakedAmountByRequestor >= amt*10**18,"Insufficient stake");
        require(thisDevice.engage == false, "The device is already engaged");
        ++requestID;
    ...
    function AcceptDeviceRequestByProvider(uint256 _reqID) public isListed(requests[_reqID].devID) {
                Request storage thisRequest = requests[_reqID];
                DeviceDetails storage thisDevice = deviceDetails[thisRequest.devID];
                require(msg.sender==thisRequest.deviceOwner,"You are not the owner of the device");
                require(
    ...
    function rejectRequestByProvider(uint256 _reqID,string memory reason)public isListed(requests[_reqID].devID){
        Request storage thisRequest = requests[_reqID];
                DeviceDetails storage thisDevice = deviceDetails[thisRequest.devID];
                require(msg.sender==thisRequest.deviceOwner,"You are not the owner of the device");
                require(
                ...
                require(
                    !thisRequest.request_end,"Request successfully completed "
                );
